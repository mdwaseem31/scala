object scala11 {
	...
	case class TempData(day:Int,year:Int,precip:Double,aveTemp:Int,maxTemp:Int,minTemp:Int);import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(327); 
	
	def parseLine(line:String):TempData = {
		val p = line.split(",").map(_.trim)
		TempData(p(1).toInt,p(4).toInt,p(5).toDouble,p(6).toInt,p(7).toInt,p(8).toInt)
	};System.out.println("""parseLine: (line: String)scala11.TempData""");$skip(41); val res$0 = 
	
	new java.io.File(".").getAbsolutePath;System.out.println("""res0: String = """ + $show(res$0));$skip(111); 
	val source = io.Source.fromFile("C:/Spring/workspace/InfiniteSkills/WorkingFiles/Chapter2/TX417945_1263.csv");System.out.println("""source  : scala.io.BufferedSource = """ + $show(source ));$skip(84); 
  ...
  //Now get the data out of Iterator and store it in memory. This can be done by converting it to an Array or a List
 	val data = lines.map(parseLine).toArray;System.out.println("""data  : Array[scala11.TempData] = """ + $show(data ));$skip(70); val res$3 = 
